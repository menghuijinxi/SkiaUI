...
`video` 的预解码只在 `preload="auto"` 或显式 prepare 后启动。FFmpeg worker 填满少量视频
帧和对应 PCM 后在条件变量上等待；Tick 只比较音频/单调时钟与帧 PTS，高频 Tick 保持当前
帧，低频 Tick 丢弃过期视觉帧。循环只额外保留最多 0.5 秒、最多 8 张首段画面及对应 PCM。
WASAPI 回调使用固定内存的单生产者/单消费者环形缓冲，不调用 FFmpeg、不分配内存，也不
